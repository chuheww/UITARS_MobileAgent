...
def get_action_prompt_uitars(instruction, perception_infos, width, height, keyboard, summary_history, action_history,
                      last_summary, last_action, add_info, error_flag, completed_content, memory):
    prompt = MOBILE_USE.format(instruction=instruction,language='English')
    # prompt += "\n\n### Screenshot Information ###\n"
    prompt += "Below are clickable elements on the current screen (format: [x,y]; text/icon description):\n"
    ...
    # prompt += "\n### Keyboard Status ###\n"
    prompt += "The keyboard is activated." if keyboard else "The keyboard is not activated.\n"

    if summary_history or action_history:
        # prompt += "\n### History Operations ###\n"
        for i, (s, a) in enumerate(zip(summary_history, action_history)):
            prompt += f"Step-{i+1}: [Thought: {s}, Action: {a}]\n"

    # if completed_content:
    #     prompt += "\n### Progress ###\n"
    #     prompt += f"Completed contents: {completed_content}\n"
    #
    # if memory:
    #     prompt += "\n### Memory ###\n"
    ...
    # if error_flag:
    #     prompt += "\n### Last Operation ###\n"
    #     prompt += f"You previously tried to perform '{last_summary}' and executed '{last_action}', but it didn't meet expectations."
    #
    # if add_info:
    #     prompt += "\n### Hint ###\n"
    #     prompt += add_info + "\n"
...
def get_action_prompt_uitars1(instruction, perception_infos, keyboard, summary_history, action_history):
    prompt = MOBILE_USE.format(instruction=instruction,language='English')
    # prompt += "\n\n### Screenshot Information ###\n"
    prompt += "Below are clickable elements on the current screen (format: [x,y]; text/icon description):\n"
    for info in perception_infos:
